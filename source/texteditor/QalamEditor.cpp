...
void QalamEditor::initializeEditor()
{
    ...
    connect(this, &QalamEditor::cursorPositionChanged, this, &QalamEditor::highlightCurrentLine);
    connect(this->document(), &QTextDocument::contentsChanged, this, &QalamEditor::updateFoldRegions);
...
void QalamEditor::wheelEvent(QWheelEvent *event) {
...
void QalamEditor::clearDiagnostics() {
    m_diagnostics.clear();
    applyEditorDecorations();
    viewport()->update();
}

void QalamEditor::setSemanticTokens(const QVector<BaaSemanticToken> &tokens)
{
    if (highlighter) highlighter->setSemanticTokens(tokens);
}

void QalamEditor::clearSemanticTokens()
{
    if (highlighter) highlighter->clearSemanticTokens();
}

void QalamEditor::setInlayHints(const QVector<BaaInlayHint> &hints)
{
    m_inlayHints = hints;
...
void QalamEditor::clearInlayHints()
{
...
void QalamEditor::clearFoldingRanges()
{
    replaceFoldRegions({});
}

void QalamEditor::useLocalFoldingRanges()
{
    updateFoldRegions();
}

void QalamEditor::applySemanticSelectionRanges(
...
void QalamEditor::expandSemanticSelection()
{
...
void QalamEditor::shrinkSemanticSelection()
{
...
void QalamEditor::updateFontSize(int size) {
    if (size < 10) {
        size = 18;
    }
...
void QalamEditor::updateFontType(QString font) {
...
void QalamEditor::toggleComment()
{
    QTextCursor cursor = textCursor();
...
void QalamEditor::duplicateLine()
{
    QTextCursor cursor = textCursor();
...
bool QalamEditor::eventFilter(QObject* obj, QEvent* event) {
...
void QalamEditor::contextMenuEvent(QContextMenuEvent *event)
{
...
void QalamEditor::updateLineNumberAreaWidth() {
...
void QalamEditor::updateLineNumberArea(const QRect &rect, int dy) {
    if (dy)
...
void QalamEditor::paintEvent(QPaintEvent *event)
{
...
void QalamEditor::resizeEvent(QResizeEvent* event) {
...
void QalamEditor::updateFoldRegions() {
    m_semanticSelectionRanges.clear();
    m_semanticSelectionHistory.clear();
...
void QalamEditor::replaceFoldRegions(const QVector<FoldRegion> &regions)
{
...
void QalamEditor::applyFoldVisibility()
{
...
void QalamEditor::toggleFold(int blockNumber) {
    for (FoldRegion &region : foldRegions) {
        if (region.startBlockNumber == blockNumber) {
...
void QalamEditor::dragMoveEvent(QDragMoveEvent* event) {
    event->acceptProposedAction();
}
...
void QalamEditor::dragLeaveEvent(QDragLeaveEvent* event) {
    event->accept();
}


void QalamEditor::mouseMoveEvent(QMouseEvent *event) {
...
void QalamEditor::leaveEvent(QEvent *event) {
    clearSemanticPresentation();
    QPlainTextEdit::leaveEvent(event);
}

void QalamEditor::scheduleLanguageHover(const QPoint &viewportPosition,
...
void QalamEditor::showLanguageHover(const BaaHover &hover,
                                int requestLine,
                                int requestCharacter)
{
...
void QalamEditor::showSignatureHelp(const BaaSignatureHelp &signatureHelp,
                                int requestLine,
                                int requestCharacter)
{
    const QTextCursor cursor = textCursor();
...
void QalamEditor::requestSignatureHelp()
{
    if (currentFilePath().isEmpty()) return;
    const QTextCursor cursor = textCursor();
...
void QalamEditor::clearSemanticPresentation()
{
    m_hoverTimer.stop();
    m_hoverRequestLine = -1;
    m_hoverRequestCharacter = -1;
...
void QalamEditor::cursorIndentation() {
    QTextCursor cursor = textCursor();
    QString lineText = cursor.block().text();
...
QString QalamEditor::getCurrentLineIndentation(const QTextCursor &cursor) const {
...
void QalamEditor::startAutoSave() {
    m_autoSave->filePath = currentFilePath();
    m_autoSave->start();
}

void QalamEditor::stopAutoSave() {
    m_autoSave->stop();
}

void QalamEditor::removeBackupFile() {
    m_autoSave->filePath = currentFilePath();
    m_autoSave->removeBackupFile();
}


void QalamEditor::updateHighlighterTheme(std::shared_ptr<SyntaxTheme> theme) {
...
void QalamEditor::setupAutoComplete() {
    model = new CompletionModel(this);
    QCompleter *completer = new QCompleter(this);
    setCompleter(completer);
}

void QalamEditor::setCompleter(QCompleter *completer) {
    ...
    // Custom Rich Popup ---
    QalamCompletionPopup *popup = new QalamCompletionPopup;
...
void QalamEditor::focusOutEvent(QFocusEvent *e) {
...
void QalamEditor::keyPressEvent(QKeyEvent *e) {
    ...
    // Bracket and quote auto-pairing (delegated to QalamBracketHandler)
    ...
    // Snippet navigation (delegated to QalamSnippetManager)
...
void QalamEditor::performCompletion(bool explicitRequest) {
...
void QalamEditor::showLanguageCompletions(const QVector<BaaCompletionItem> &items,
                                      int line,
                                      int character)
{
    const QTextCursor cursor = textCursor();
...
void QalamEditor::showCompletionPopup()
{
    QRect cr = cursorRect();
